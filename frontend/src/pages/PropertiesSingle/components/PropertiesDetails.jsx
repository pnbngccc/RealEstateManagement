...
function PropertyDetail() {
    ...
    try {
      if (isFavorite) {
        ...
        if (response.data.success) {
          ...
          setIsFavorite(false);
          setFavoriteId(null);
          // Gọi lại hàm kiểm tra trạng thái yêu thích
        ...
        if (response.data.success) {
          alert("Đã thêm vào danh sách yêu thích!");
          setIsFavorite(true);
          setFavoriteId(response.data.favoriteId); // Giả sử API trả về favoriteId
        } else {
          alert("Lỗi khi lưu tài sản vào danh sách yêu thích.");
        }
      }
  ...
  const checkFavoriteStatus = async () => {
    ...
    if (!property || !property._id) {
      console.error("Property is not available or has no ID");
      return; // Dừng lại nếu property không có hoặc không có ID
    }
    ...
    try {
      ...
      console.log("Phản hồi từ API thêm yêu thích:", response.data); // Thêm log này để kiểm tra

      if (response.data.success) {
        const favorite = response.data.data.find(
          (item) => item.idProperty && item.idProperty._id === property._id
        );
        ...
        if (favorite) {
          ...
          setIsFavorite(false); // Không phải yêu thích
        }
      }
    } catch (error) {
      console.error("Lỗi khi kiểm tra trạng thái yêu thích:", error);
  ...
  useEffect(() => {
    if (property) {
      checkFavoriteStatus(); // Chỉ gọi khi property có giá trị hợp lệ
    }
  }, [property]);
      ...
      <div className="details">
        <div className="wrapper">
          ...
          <div className="info">
            ...
            <div className="mt-3">
              <p>{property.description || "Mô tả không có."}</p>
            </div>
          </div>
        </div>
      </div>
